...
export default function ResetPasswordScreen() {
  ...
  useEffect(() => {
    ...
    async function init() {
      // 1. App was cold-launched by the deep link
      ...
      // 2. App was already foregrounded — listen for the URL event
      urlSub = Linking.addEventListener('url', async ({ url }) => {
        const ok = await applyTokensFromUrl(url);
        setStage(ok ? 'form' : 'error');
      });

      // Fallback: give up after 10 s
      timeoutId = setTimeout(() => setStage('error'), 10_000);
    }
      ...
      <View style={s.body}>
        ...
        <Animated.View style={[s.fieldsBlock, { transform: [{ translateX: shake }] }]}>
          ...
          <View style={s.fieldGroup}>
            ...
            <View style={[s.fieldWrap, { backgroundColor: theme.card, borderColor: fieldError ? Colors.error : theme.border }]}>
